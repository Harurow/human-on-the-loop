...
| コードレビュー（タスク毎） | サブエージェント（fresh context） | diff + タスク行（完了条件）+ 関連仕様・設計抜粋 + 費用上限の NR 行（あれば）+ checklist |
| 整合性監査（done 前） | サブエージェント（fresh context） | docs/ 一式 + git log |
| 最終受け入れ検証 | サブエージェント（fresh context） | requirements の受け入れ基準 + 成果物のみ |
| セキュリティレビュー（最終受け入れ検証 (a)） | サブエージェント（fresh context） | リポジトリ + 要件のセキュリティ前提（NR）+ design.md + checklist-security |
...
- **inbox の消化**: `docs/inbox.md`（hotl-pm が人間の指示を届ける受け渡しファイル）が存在し内容があれば、**読んだ項目だけ**を trail に `interrupt` として転記して inbox から削除し（読取後に追記された新項目を消さない）、reporting.md の割り込み手順で分類・反映**してから**フェーズ実行に進む。項目が複数ある場合はまとめて読み、**新しい指示が古い指示を上書きするなら（停止→再開など）最新の意図に従う**（全項目の転記は行う）。**意図の比較対象は人間のメッセージのみ**: このターンの起動メッセージが人間の発話なら比較に含める（最新の人間の意図が常に勝ち、陳腐化した停止指示が新しい要望を妨げてはならない）。**hotl-pm からの委譲プロンプト（「続きから」等）は人間の発話ではないため比較に含めない** — 委譲プロンプトが人間の停止指示を上書きすることはない。単純な再開指示（「進めて」等）は転記時に対応記録（対応: 再開）を、**後続の指示に上書きされた項目**には対応記録（対応: 後続指示により陳腐化）を、それぞれ同時に追記する（監査の未処理判定に残さない）。また trail 末尾に**対応記録の無い interrupt** が残っている場合も同様に、フェーズ実行の前に割り込み手順で処理する（単純再開と誤判定して旧方針のまま進まない）
