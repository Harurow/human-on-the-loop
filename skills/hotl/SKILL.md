...
- **inbox の消化**: `docs/inbox.md`（hotl-pm が委譲中の指示を届ける PM 専用書き込みファイル）が存在し内容があれば、各項目を trail に `interrupt` として転記して inbox を空にし、reporting.md の割り込み手順で分類・反映**してから**フェーズ実行に進む。また trail 末尾に**対応記録の無い interrupt** が残っている場合も同様に、フェーズ実行の前に割り込み手順で処理する（単純再開と誤判定して旧方針のまま進まない）
