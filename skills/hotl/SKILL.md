...
- **inbox の消化**: `docs/inbox.md`（hotl-pm が人間の指示を届ける受け渡しファイル）が存在し内容があれば、フェーズ実行の**前に**次を行う:
  ...
  - reporting.md の割り込み手順で分類・反映する。**新しい指示が古い指示を上書きするなら（停止→再開など）最新の意図に従う**。比較対象は**人間のメッセージのみ**: このターンの起動メッセージが人間の発話なら含める（最新の人間の意図が常に勝ち、陳腐化した停止指示が新しい要望を妨げてはならない）。**hotl-pm の委譲プロンプト（「続きから」等）は人間の発話ではないため含めない** — 委譲プロンプトが人間の停止指示を上書きすることはない
  - 転記と同時に対応記録を追記する: 単純な再開指示（「進めて」等）には「対応: 再開」、後続の指示に上書きされた項目には「対応: 後続指示により陳腐化」（監査の未処理判定に残さない）
  - trail 末尾に**対応記録の無い interrupt** が残っている場合も同様に、フェーズ実行の前に割り込み手順で処理する（単純再開と誤判定して旧方針のまま進まない）
