...
- **要件定義**: ID・受け入れ基準・出所（ユーザー / 提案）付きの要件に落とす。非機能5観点（性能・費用・セキュリティ前提・データ保全・運用）と前提・制約（実行環境・既存資産・外部サービス）は未確認でも既定値を「提案」として埋めて承認時に確認する。fresh context のレビュアーで矛盾・曖昧さ・テスト可能性・MECE をチェック
- **承認ゲート**: 人間が要件を承認。承認は requirements.md の sha256 付きで記録され、承認後に要件が書き換わると自律続行を拒否してゲートを再実行する
- **自律区間**: 仕様化（トレーサビリティ確認）→ 設計（技術選定・検証ツールチェーン選定・自己検証）→ 開発（タスク分解 → 実装 → ツールチェーン検証 → セルフレビュー → 独立コードレビューのループ、done 前に整合性監査＋UX ウォークスルー〔利用者視点〕＋受け入れ検証〔機能基準に加え、セキュリティ攻撃面・性能実測・費用実測の非機能横断チェック〕）。タスク3件ごとに中間報告。指摘は3分類（自律修正 / バックログ / BLOCKED）で処理
