...
- **自律区間**: 仕様化（トレーサビリティ確認）→ 設計（技術選定・検証ツールチェーン選定・自己検証）→ 開発（タスク分解 → 実装 → ツールチェーン検証 → セルフレビュー → 独立コードレビューのループ、done 前に整合性監査＋受け入れ検証〔機能基準に加え、セキュリティ攻撃面・性能実測・費用実測の非機能横断チェック〕）。タスク3件ごとに中間報告。指摘は3分類（自律修正 / バックログ / BLOCKED）で処理
